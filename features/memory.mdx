...
1. **Automatic storage** — When you accept file edits in agentic mode, the decision is stored
2. **Semantic search** — New prompts are matched against stored memories
3. **Context injection** — Relevant memories are injected into prompts

## Storage

Memory uses SQLite FTS5 (zero dependencies):
...
When Ollama is available, PuzldAI uses embeddings for better semantic search:
